...
public class SqlExecutor {
	 ...
	 * 
	 * @param session - the session scope
	 * @return - a List of BatchResult objects (may be null if no batch has been initiated). There will be one BatchResult object in the list for each sub-batch
	 *         executed
	 ...
	 * @throws SQLException - if the procedure fails
	 */
	public void executeQueryProcedure(RequestScope request, Connection conn, String sql, Object[] parameters, int skipResults, int maxResults,
			RowHandlerCallback callback) throws SQLException {
		ErrorContext errorContext = request.getErrorContext();
    ...
    private ResultSet handleMultipleResults(PreparedStatement ps, RequestScope request, int skipResults, int maxResults, RowHandlerCallback callback)
			throws SQLException {
		ResultSet rs;
	...
	private void retrieveOutputParameters(RequestScope request, CallableStatement cs, ParameterMapping[] mappings, Object[] parameters,
			RowHandlerCallback callback) throws SQLException {
		String tagCode = null;
		for (int i = 0; i < mappings.length; i++) {
			...
			if (mapping.isOutputAllowed()) {
				if ("java.sql.ResultSet".equalsIgnoreCase(mapping.getJavaTypeName())) {
					if (TagCodeEnum.SUCCESS.equals(tagCode))// 如果错误码是00，按照原来的逻辑
					{
						...
						try {
							...
							// 如果错误码不是00，继续做其他mapping但不做游标的mapping。尝试关闭游标但忽略错误（因为有时候可能游标合法，关闭可以释放内存）
						}
					}
				} else {
					parameters[i] = mapping.getTypeHandler().getResult(cs, i + 1);
					// 获取错误码,要保证o_tagcode在xml中是第一个out参数
					if (mapping.getPropertyName().equalsIgnoreCase("TagCode"))
						tagCode = (String) parameters[i];
				}
			}
		}
	}
	...
	private static class Batch {
		...
		/**
		 * TODO (Jeff Butler) - maybe this method should be deprecated in some release, and then removed in some even later release. executeBatchDetailed gives
