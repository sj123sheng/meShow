...
public class NewsV2Functions {
	 ...
	 * @return 结果字符串
	 */
	public JsonObject addNews(JsonObject jsonObject, boolean checkTag,
		...
		try {
			appId = CommonUtil.getJsonParamInt(jsonObject, "a",
					AppIdEnum.AMUSEMENT, null, 1, Integer.MAX_VALUE);
			platform = CommonUtil.getJsonParamInt(jsonObject, "platform", 0,
					null, 1, Integer.MAX_VALUE);
			userId = CommonUtil.getJsonParamInt(jsonObject, "userId", 0,
					"06020001", 1, Integer.MAX_VALUE);
			// 媒体类型
			mediaType = CommonUtil.getJsonParamInt(jsonObject, "mediaType", 0,
					null, 1, Integer.MAX_VALUE);
			content = CommonUtil.getJsonParamString(jsonObject, "content",
					null, null, 1, 500);
			if (content != null) {
			...
			mediaUrl = CommonUtil.getJsonParamString(jsonObject, "mediaUrl",
					null, null, 1, Integer.MAX_VALUE);
			if (!StringUtil.strIsNull(mediaUrl)) {
				mediaUrl = mediaUrl.replaceFirst(
						ConfigHelper.getMediahttpdir(), "");
				mediaUrl = mediaUrl.replaceFirst("/kktv", "");
			}

			imageUrl = CommonUtil.getJsonParamString(jsonObject, "imageUrl",
					null, null, 1, Integer.MAX_VALUE);

			mediaDur = CommonUtil.getJsonParamInt(jsonObject, "mediaDur", -1,
					null, 1, Integer.MAX_VALUE);
			if (mediaDur > 7200) {
			...
			videoTitle = CommonUtil.getJsonParamString(jsonObject,
					"videoTitle", null, null, 1, 40);

			// 1-upyun 2-qiniu
			mediaFrom = CommonUtil.getJsonParamInt(jsonObject, "mediaFrom", 1,
					null, 1, Integer.MAX_VALUE);

			resType = CommonUtil.getJsonParamInt(jsonObject, "newsType", 8,
					null, 1, Integer.MAX_VALUE);

			topic = CommonUtil.getJsonParamString(jsonObject, "topic", null,
					null, 1, 10);
		...
		if (mediaType == NewsMediaTypeEnum.AUDIO
				|| mediaType == NewsMediaTypeEnum.VIDEO) {
			...
			resource.setImageUrl(imageUrl != null ? imageUrl : Pattern
					.compile("mp4$").matcher(mediaUrl).replaceAll("jpg"));
	 ...
	 * @return 结果字符串
	 */
	public JsonObject deleteNews(JsonObject jsonObject, boolean checkTag,
			HttpServletRequest request) throws Exception {
		// 该接口需要验证token,未验证的返回错误码
		if (!checkTag) {
			JsonObject result = new JsonObject();
		...
		// 解析参数
		try {
			userId = CommonUtil.getJsonParamInt(jsonObject, "userId", 1,
					"06030002", 1, Integer.MAX_VALUE);
			newsId = CommonUtil.getJsonParamInt(jsonObject, "newsId", 1,
					"06030004", 1, Integer.MAX_VALUE);
		...
		if (NewsService.deleteNews(newsId, userId)) {
			...
			// 查询该动态是否为mongo中最新动态
			DBObject newsDBObj = (DBObject) CommonDB
					.getInstance(CommonDB.CACHEDB)
					.getCollection(CollectionEnum.USERLATESTNEWS)
					.findOne(new BasicDBObject("userId", userId));
			int lastNewsId = 0;
			if (newsDBObj != null && newsDBObj.containsField("latestNews")
					&& newsDBObj.get("latestNews") != null) {
				String str = (String) newsDBObj.get("latestNews");
				try {
					JsonObject json = new JsonParser().parse(str)
							.getAsJsonObject();
			...
			if (lastNewsId == newsId) {
				...
				if (resourceService != null) {
					List<NewsInfo> lastNews = NewsService.getSelfNewsList(
							userId, 1, 1, 0, 0);
					if (lastNews != null && lastNews.size() > 0
						...
						if (newsInfo.getTopicId() != null) {
							lastNewsJson.addProperty("topicId",
									temp.getTopicId());
						}
						lastNewsJson.addProperty("content", temp.getContent());
						lastNewsJson.addProperty("publishedTime", temp
								.getPublishedTime().getTime());
						if (temp.getRefVideo() == null) {
							JsonObject mediaSource = new JsonObject();
							// 图片类型存一张
							mediaSource.addProperty("mediaType", 1);// 1-图片 3-视频
							String[] urls = getRegexAdmin(temp.getRefImage())
									.split(",");
							mediaSource.addProperty("picCount", urls.length);
							Resource resource = resourceService.getResource(
									Integer.valueOf(urls[0]), 1);
							mediaSource.addProperty("imageUrl_1280",
									resource.getImageUrl());
							mediaSource.addProperty("imageUrl_272",
									resource.getImageUrl());
							mediaSource.addProperty("imageUrl_128",
									resource.getImageUrl());
							lastNewsJson.addProperty("mediaSource",
									new Gson().toJson(mediaSource));
						} else {
							JsonObject mediaSource = new JsonObject();
							...
							Resource resource = resourceService.getResource(
									Integer.valueOf(getRegexAdmin(temp
											.getRefVideo())), 3);
							mediaSource.addProperty("imageUrl_1280",
									resource.getImageUrl());
							mediaSource.addProperty("imageUrl_272",
						...
						updateDBObj.put("latestNews",
								new Gson().toJson(lastNewsJson));
						CommonDB.getInstance(CommonDB.CACHEDB)
								.getCollection(CollectionEnum.USERLATESTNEWS)
						...
						CommonDB.getInstance(CommonDB.CACHEDB)
								.getCollection(CollectionEnum.USERLATESTNEWS)
								.remove(new BasicDBObject("userId", userId));
					}
				}
	 ...
	 * @return 结果字符串
	 */
	public JsonObject getUserNewsList(JsonObject jsonObject, boolean checkTag,
			HttpServletRequest request) throws Exception {
		...
		// 解析参数
		try {
			userId = CommonUtil.getJsonParamInt(jsonObject, "userId", 0, null,
					1, Integer.MAX_VALUE);
			orderId = CommonUtil.getJsonParamInt(jsonObject, "orderId", 0,
					"06040001", 1, Integer.MAX_VALUE);
			start = CommonUtil.getJsonParamInt(jsonObject, "start", 0, null, 0,
					Integer.MAX_VALUE);
			offset = CommonUtil.getJsonParamInt(jsonObject, "offset",
					Constant.return_news_count, null, 1, Integer.MAX_VALUE);
			platform = CommonUtil.getJsonParamInt(jsonObject, "platform", 0,
		...
		if (count > 0) {
			// 登录返回是否点赞过
			List<NewsInfo> newsList = NewsService.getSelfNewsList(orderId,
					start, offset, isSelf, checkTag ? userId : 0);
			if (newsList != null && newsList.size() > 0) {
				JsonArray jNewsList = new JsonArray();
				for (NewsInfo newsInfo : newsList) {
					JsonObject json = NewsService.getNewResourceJson(newsInfo,
							platform, false);
					jNewsList.add(json);
				}
				...
				if (actorInfo != null) {
					...
					if (actorInfo.getPortrait() != null) {
						if (platform == PlatformEnum.WEB) {
							result.addProperty("portrait_path_256",
									actorInfo.getPortrait() + "!256");
						} else if (platform == PlatformEnum.ANDROID) {
							result.addProperty("portrait_path_48",
									actorInfo.getPortrait() + "!48");
							result.addProperty("portrait_path_128",
									actorInfo.getPortrait() + "!128");
							...
							result.addProperty("portrait_path_1280",
									actorInfo.getPortrait() + "!1280");
							result.addProperty("portrait_path_256",
									actorInfo.getPortrait() + "!256");
							result.addProperty("portrait_path_128",
									actorInfo.getPortrait() + "!128");
					...
					result.addProperty(
							"isLive",
							actorInfo.getLiveStarttime() != null
									&& actorInfo.getLiveEndtime() == null ? 1
									: 0);
				...
				// 返回关注关系
				result.addProperty("isFollowed",
						com.melot.kktv.service.UserRelationService.isFollowed(
								userId, orderId) ? 1 : 0);
				...
				result.addProperty("mediaPathPrefix",
						ConfigHelper.getMediahttpdir()); // 多媒体前缀
				result.addProperty("videoPathPrefix",
						ConfigHelper.getVideoURL());// 七牛前缀
				result.addProperty("TagCode", TagCodeEnum.SUCCESS);
				return result;
			} else {
				result.addProperty("TagCode", TagCodeEnum.IRREGULAR_RESULT);
	 ...
	 * @return 结果字符串
	 */
	public JsonObject addNewsComment(JsonObject jsonObject, boolean checkTag,
			HttpServletRequest request) throws Exception {
		...
		try {
			userId = CommonUtil.getJsonParamInt(jsonObject, "userId", 0,
					"06050001", 1, Integer.MAX_VALUE);
			newsId = CommonUtil.getJsonParamInt(jsonObject, "newsId", 0,
					"06050003", 1, Integer.MAX_VALUE);
			toUserId = CommonUtil.getJsonParamInt(jsonObject, "toUserId", 0,
					null, 1, Integer.MAX_VALUE);
			platform = CommonUtil.getJsonParamInt(jsonObject, "platform", 0,
					null, 1, Integer.MAX_VALUE);
			content = CommonUtil.getJsonParamString(jsonObject, "content",
	 ...
	 * @throws Exception
	 */
	public JsonObject addPraise(JsonObject jsonObject, boolean checkTag,
			HttpServletRequest request) throws Exception {
		JsonObject result = new JsonObject();
		...
		try {
			userId = CommonUtil.getJsonParamInt(jsonObject, "userId", 0,
					"06190001", 1, Integer.MAX_VALUE);
			commentId = CommonUtil.getJsonParamInt(jsonObject, "commentId", 0,
					"06190003", 1, Integer.MAX_VALUE);
		} catch (ErrorGetParameterException e) {
			result.addProperty("TagCode", e.getErrCode());
			return result;
		}
	 ...
	 * @throws Exception
	 */
	public JsonObject getRecommendTopicOrComment(JsonObject jsonObject,
			boolean checkTag, HttpServletRequest request) throws Exception {
		JsonObject result = new JsonObject();
		...
		try {
			// 0-短评, 1-话题
			type = CommonUtil.getJsonParamInt(jsonObject, "type", 0, null, 0,
					Integer.MAX_VALUE);
			count = CommonUtil.getJsonParamInt(jsonObject, "count", 6, null, 0,
					Integer.MAX_VALUE);
		} catch (ErrorGetParameterException e) {
			result.addProperty("TagCode", e.getErrCode());
			return result;
		...
		if (type == 0) {
			Set<String> commentList = NewsV2Source.getRecommendComment(0,
					count - 1);
			...
			for (String temp : topicList) {
				JsonObject json = new JsonParser().parse(temp)
						.getAsJsonObject();
				topicArray.add(json);
			}
	 ...
	 * @throws Exception
	 */
	public JsonObject changeHotComments(JsonObject jsonObject,
			boolean checkTag, HttpServletRequest request) throws Exception {
		JsonObject result = new JsonObject();
		...
		if (commentList != null && commentList.size() > 0) {
			...
			if (length <= 12) {
				...
				while (set.size() < 12) {
					...
					if (!set.contains(i)) {
						hotCommentList.add(new JsonParser().parse(list[i])
								.getAsJsonObject());
						set.add(i);
					}
				}
	 ...
	 * @throws Exception
	 */
	public JsonObject getHotNews(JsonObject jsonObject, boolean checkTag,
			HttpServletRequest request) throws Exception {
		JsonObject result = new JsonObject();
		...
		try {
			userId = CommonUtil.getJsonParamInt(jsonObject, "userId", 0, null,
					1, Integer.MAX_VALUE);
			start = CommonUtil.getJsonParamInt(jsonObject, "start", 0, null, 1,
					Integer.MAX_VALUE);
			offset = CommonUtil.getJsonParamInt(jsonObject, "offset", 10, null,
					1, Integer.MAX_VALUE);
			//1-video 0-default news
			type = CommonUtil.getJsonParamInt(jsonObject, "type", 0, null,
					1, Integer.MAX_VALUE);
		} catch (ErrorGetParameterException e) {
			result.addProperty("TagCode", e.getErrCode());
			return result;
		...
		if (type > 0) {
			...
			if (hotTopic != null && hotTopic.size() > 0) {
				...
				for (Tuple tuple : hotTopic) {
					try {
						JsonObject json = new JsonParser().parse(tuple.getElement())
								.getAsJsonObject();
		...
		if (hotNews != null && hotNews.size() > 0) {
			...
			if (type == 1 && hotNewsList.size() < (offset - containSize)) {
				...
				getJson(checkTag, userId, offset - containSize - size, otherNews, hotNewsList);
			} 
	 ...
	 * @throws Exception
	 */
	public JsonObject getTopicPage(JsonObject jsonObject, boolean checkTag,
			HttpServletRequest request) throws Exception {
		JsonObject result = new JsonObject();
		...
		// 解析参数
		try {
			sortType = CommonUtil.getJsonParamInt(jsonObject, "sortType", 0,
					null, 0, Integer.MAX_VALUE);
			topicId = CommonUtil.getJsonParamInt(jsonObject, "topicId", 0,
					"06230001", 1, Integer.MAX_VALUE);
			userId = CommonUtil.getJsonParamInt(jsonObject, "userId", 0, null,
					1, Integer.MAX_VALUE);
			start = CommonUtil.getJsonParamInt(jsonObject, "start", 0, null, 0,
					Integer.MAX_VALUE);
			...
			for (String str : hotTopic) {
				try {
					...
					if (json.get("topicId").getAsInt() == topicId) {
						result.addProperty("imageUrl", json.get("imageUrl")
								.getAsString());
						result.addProperty("introduction",
								json.get("introduction").getAsString());
						break;
					}
				} catch (Exception e) {
				}
		...
		if (count > 0) {
			List<NewsInfo> newsList = NewsService.getNewsListByTopicId(topicId,
					sortType, start, offset, checkTag ? userId : 0);
			if (newsList != null && newsList.size() > 0) {
				JsonArray jNewsList = new JsonArray();
				for (NewsInfo newsInfo : newsList) {
					JsonObject json = NewsService.getNewResourceJson(newsInfo,
					...
					json.addProperty(
							"isFollowed",
							com.melot.kktv.service.UserRelationService
									.isFollowed(userId, newsInfo.getUserId()) ? 1
									: 0);
					jNewsList.add(json);
				}
				result.add("newsList", jNewsList);
	 ...
	 * @throws Exception
	 */
	public JsonObject getCommentPage(JsonObject jsonObject, boolean checkTag,
			HttpServletRequest request) throws Exception {
		JsonObject result = new JsonObject();
		...
		if (count > 0) {
			List<NewsInfo> newsList = NewsService.getNewsListByComment(content,
					sortType, start, offset, checkTag ? userId : 0);
			if (newsList != null && newsList.size() > 0) {
				JsonArray jNewsList = new JsonArray();
				for (NewsInfo newsInfo : newsList) {
	 ...
	 * @throws Exception
	 */
	public JsonObject getStateByNewId(JsonObject jsonObject, boolean checkTag,
			HttpServletRequest request) throws Exception {
		JsonObject result = new JsonObject();
		...
		try {
			userId = CommonUtil.getJsonParamInt(jsonObject, "userId", 0, null,
					0, Integer.MAX_VALUE);
			newsIds = CommonUtil.getJsonParamString(jsonObject, "newsIds",
					null, "06250001", 1, Integer.MAX_VALUE);
			platform = CommonUtil.getJsonParamInt(jsonObject, "platform", 0,
					null, Integer.MIN_VALUE, Integer.MAX_VALUE);
		} catch (ErrorGetParameterException e) {
			result.addProperty("TagCode", e.getErrCode());
	 ...
	 * @throws Exception
	 */
	public JsonObject addNewsPraise(JsonObject jsonObject, boolean checkTag,
			HttpServletRequest request) throws Exception {
		JsonObject result = new JsonObject();
		...
		try {
			userId = CommonUtil.getJsonParamInt(jsonObject, "userId", 0,
					"06260001", 1, Integer.MAX_VALUE);
			newsId = CommonUtil.getJsonParamInt(jsonObject, "newsId", 0,
					"06260003", 1, Integer.MAX_VALUE);
		} catch (ErrorGetParameterException e) {
			result.addProperty("TagCode", e.getErrCode());
			return result;
		}
	 ...
	 * @throws Exception
	 */
	public JsonObject cancelNewsPraise(JsonObject jsonObject, boolean checkTag,
			HttpServletRequest request) throws Exception {
		JsonObject result = new JsonObject();
		...
		try {
			userId = CommonUtil.getJsonParamInt(jsonObject, "userId", 0,
					"06270001", 1, Integer.MAX_VALUE);
			newsId = CommonUtil.getJsonParamInt(jsonObject, "newsId", 0,
					"06270003", 1, Integer.MAX_VALUE);
		} catch (ErrorGetParameterException e) {
			result.addProperty("TagCode", e.getErrCode());
			return result;
		}
	 ...
	 * @throws Exception
	 */
	public JsonObject cancelPraise(JsonObject jsonObject, boolean checkTag,
			HttpServletRequest request) throws Exception {
		JsonObject result = new JsonObject();
		...
		try {
			userId = CommonUtil.getJsonParamInt(jsonObject, "userId", 0,
					"06280001", 1, Integer.MAX_VALUE);
			commentId = CommonUtil.getJsonParamInt(jsonObject, "commentId", 0,
					"06280003", 1, Integer.MAX_VALUE);
		} catch (ErrorGetParameterException e) {
			result.addProperty("TagCode", e.getErrCode());
			return result;
		}
	 ...
	 * @throws Exception
	 */
	public JsonObject getCommentListByTopic(JsonObject jsonObject, boolean checkTag,
			HttpServletRequest request) throws Exception {
		JsonObject result = new JsonObject();
	 ...
	 * @return 结果字符串
	 */
	public JsonObject deleteNewsComment(JsonObject jsonObject,
			boolean checkTag, HttpServletRequest request) throws Exception {
		...
		try {
			userId = CommonUtil.getJsonParamInt(jsonObject, "userId", 0,
					"06060001", 1, Integer.MAX_VALUE);
			commentId = CommonUtil.getJsonParamInt(jsonObject, "commentId", 0,
					"06060003", 1, Integer.MAX_VALUE);
		} catch (ErrorGetParameterException e) {
			result.addProperty("TagCode", e.getErrCode());
			return result;
		}
		...
		// 2.if the news exist
		NewsInfo newsInfo = NewsService.getNewsInfoById(
				newsCommentHist.getNewsId(), 0);
		if (newsInfo == null) {
			result.addProperty("TagCode", "06060104");
			return result;
		}
		// 3.if the userId is news or comment owner
		if (userId != newsCommentHist.getUserId()
				&& userId != newsInfo.getUserId()) {
	 ...
	 * @throws Exception
	 */
	public JsonObject getNewsComment(JsonObject jsonObject, boolean checkTag,
			HttpServletRequest request) throws Exception {
		JsonObject result = new JsonObject();
		...
		try {
			userId = CommonUtil.getJsonParamInt(jsonObject, "userId", 0, null,
					1, Integer.MAX_VALUE);
			newsId = CommonUtil.getJsonParamInt(jsonObject, "newsId", 1,
					"06070002", 1, Integer.MAX_VALUE);
			start = CommonUtil.getJsonParamInt(jsonObject, "start", 0,
					"06070003", 0, Integer.MAX_VALUE);
			offset = CommonUtil.getJsonParamInt(jsonObject, "offset", 1,
					"06070004", 1, Integer.MAX_VALUE);
		} catch (ErrorGetParameterException e) {
			result.addProperty("TagCode", e.getErrCode());
			return result;
		}

		List<NewsCommentHist> commentList = NewsService.getCommentList(newsId,
				start, offset, userId);
		if (commentList != null && commentList.size() > 0) {
			JsonArray commentArray = new JsonArray();
	 ...
	 * @return 结果字符串
	 */
	public JsonObject getNewsInfoByNewsId(JsonObject jsonObject,
			boolean checkTag, HttpServletRequest request) throws Exception {
		...
		NewsInfo newsInfo = NewsService.getNewsInfoByNewsIdForState(newsId,
				checkTag ? userId : 0);
		if (newsInfo == null) {
		...
		if (actorInfo != null) {
			...
			result.addProperty("richLevel", actorInfo.getRichLevel());
			// 直播状态
			result.addProperty("isLive", actorInfo.getLiveStarttime() != null
					&& actorInfo.getLiveEndtime() == null ? 1 : 0);
			result.addProperty("roomSource", actorInfo.getRoomSource());
			result.addProperty("screenType", actorInfo.getScreenType());
			result.addProperty("actorTag", 1);
		} else {
			UserProfile userInfo = com.melot.kktv.service.UserService
					.getUserInfoV2(newsInfo.getUserId());
			if (userInfo != null) {
				result.addProperty("nickname", userInfo.getNickName());
				result.addProperty("gender", userInfo.getGender());
		...
		// 返回关注关系
		result.addProperty("isFollowed",
				com.melot.kktv.service.UserRelationService.isFollowed(userId,
						newsInfo.getUserId()) ? 1 : 0);
		result.addProperty("pathPrefix", ConfigHelper.getHttpdir()); // 图片前缀
		result.addProperty("mediaPathPrefix", ConfigHelper.getMediahttpdir()); // 多媒体前缀
		result.addProperty("videoPathPrefix", ConfigHelper.getVideoURL());// 七牛前缀
		result.addProperty("TagCode", TagCodeEnum.SUCCESS);
	 ...
	 * @return
	 */
	public JsonObject getHotMediaNewsList(JsonObject jsonObject,
			boolean checkTag, HttpServletRequest request) {
		...
		try {
			userId = CommonUtil.getJsonParamInt(jsonObject, "userId", 0, null,
					0, Integer.MAX_VALUE);
			platform = CommonUtil.getJsonParamInt(jsonObject, "platform",
					PlatformEnum.WEB, null, Integer.MIN_VALUE,
					Integer.MAX_VALUE);
			pageIndex = CommonUtil.getJsonParamInt(jsonObject, "pageIndex", 1,
					null, 1, Integer.MAX_VALUE);
			countPerPage = CommonUtil.getJsonParamInt(jsonObject,
					"countPerPage", Constant.return_news_count, null, 0,
					Integer.MAX_VALUE);
		} catch (ErrorGetParameterException e) {
			result.addProperty("TagCode", e.getErrCode());
			return result;
		...
		if (hotNews != null && hotNews.size() > 0) {
			...
			for (String str : hotNews) {
				try {
					NewsInfo newsInfo = NewsService.getNewsInfoById(
							Integer.valueOf(str), checkTag ? userId : 0);
					if (newsInfo == null) {
						continue;
					}
					JsonObject json = NewsService.getNewResourceJson(newsInfo,
		...
		if (recommendNews != null && recommendNews.size() > 0) {
			...
			while (flag-- > 0) {
				...
				try {
					NewsInfo newsInfo = new Gson().fromJson(str,
							new TypeToken<NewsInfo>() {
							}.getType());
					// 判断是否被删除了
					if (newsInfo == null) {
						continue;
					}
					NewsInfo stateInfo = NewsService.getNewsInfoById(
							Integer.valueOf(newsInfo.getNewsId()), checkTag ? userId : 0);
					if (stateInfo == null) {
						continue;
					}
					int orderId = newsInfo.getUserId();
					JsonObject json = NewsService.getNewResourceJson(stateInfo,
							1, true);
					json.addProperty("isFollowed",
							com.melot.kktv.service.UserRelationService
									.isFollowed(userId, orderId) ? 1 : 0);
	 ...
	 * @return
	 */
	public JsonObject getRecNewsList(JsonObject jsonObject, boolean checkTag,
		...
		try {
			// 解析参数
			userId = CommonUtil.getJsonParamInt(jsonObject, "userId", 1,
					TagCodeEnum.USERID_MISSING, 0, Integer.MAX_VALUE);
			start = CommonUtil.getJsonParamInt(jsonObject, "start", 0, null, 0,
					Integer.MAX_VALUE);
			offset = CommonUtil.getJsonParamInt(jsonObject, "offset",
					Constant.return_news_count, null, 1, Integer.MAX_VALUE);
		...
		// 登录返回是否点赞过
		List<NewsInfo> newsList = NewsService.getFollowNewsList(userId, start,
				offset);
		if (newsList != null && newsList.size() > 0) {
			JsonArray jNewsList = new JsonArray();
			for (NewsInfo newsInfo : newsList) {
			...
			if (start <= 0 && userId > 0) {
				NewsSource.setNewsRemindTime(String.valueOf(userId),
			...
			// 判断下页是否有数据
			List<NewsInfo> list = NewsService.getFollowNewsList(userId, start
					+ offset, offset);
	 ...
	 * @return
	 */
	public JsonObject searchNewsList(JsonObject jsonObject, boolean checkTag,
			HttpServletRequest request) {
		JsonObject result = new JsonObject();

		String key;
		try {
			key = CommonUtil.getJsonParamString(jsonObject, "key", null,
					"00440001", 0, 10);
		} catch (CommonUtil.ErrorGetParameterException e) {
			result.addProperty("TagCode", e.getErrCode());
			return result;
	 ...
	 * @return
	 */
	public JsonObject getHotWordList(JsonObject jsonObject, boolean checkTag,
			HttpServletRequest request) {
		JsonObject result = new JsonObject();
	 ...
	 * @return
	 */
	public JsonObject getPopularTopicList(JsonObject jsonObject,
			boolean checkTag, HttpServletRequest request) {
		JsonObject result = new JsonObject();
		int start;
		int offset;
		try {
			start = CommonUtil.getJsonParamInt(jsonObject, "start", 0, null, 0,
					Integer.MAX_VALUE);
			offset = CommonUtil.getJsonParamInt(jsonObject, "offset", 1, null,
					1, Integer.MAX_VALUE);
		} catch (CommonUtil.ErrorGetParameterException e) {
			result.addProperty("TagCode", e.getErrCode());
			return result;
	...
	private void getJson(boolean checkTag, int userId, int offset,
			Set<String> hotNews, JsonArray hotNewsList) {
		for (String str : hotNews) {
			try {
				NewsInfo newsInfo = NewsService.getNewsInfoById(
						Integer.valueOf(str), checkTag ? userId : 0);
