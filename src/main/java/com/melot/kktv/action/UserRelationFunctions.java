...
//				logger.error("用户"+userId+"已经关注用户"+followedId+".");
    ...
    private static Map<String, Object> interceptAndSortListBylive(int userId, int platform, List<ActorRelation> list, int type, int pageIndex, int countPerPage) {
        if (list != null && list.size() > 0) {
            ...
            if (UserRelationSource.isKeyExist("manage_list_" + userId)) {
                ...
                if (json != null) {
                    map.put("list", new Gson().fromJson(json, new com.google.gson.reflect.TypeToken<List<Room>>() {
                    }.getType()));
                    return map;
                }
            }
     ...
     * @return 结果字符串
     */
    public JsonObject getUserFollowedListByRoomSource(JsonObject jsonObject, boolean checkTag, HttpServletRequest request) throws Exception {
        JsonObject result = new JsonObject();

        @SuppressWarnings("unused")
        int userId, pageIndex, countPerPage, platform, appId, channel, roomSource;
        ...
        try {
            userId = CommonUtil.getJsonParamInt(jsonObject, "userId", 0, TagCodeEnum.USERID_MISSING, 1, Integer.MAX_VALUE);
            roomSource = CommonUtil.getJsonParamInt(jsonObject, "roomSource", 0, null, 1, Integer.MAX_VALUE);
            platform = CommonUtil.getJsonParamInt(jsonObject, "platform", 0, null, 1, Integer.MAX_VALUE);
            channel = CommonUtil.getJsonParamInt(jsonObject, "c", AppChannelEnum.KK, null, 0, Integer.MAX_VALUE);
        } catch (CommonUtil.ErrorGetParameterException e) {
            result.addProperty("TagCode", e.getErrCode());
        ...
        com.melot.kkcore.relation.service.UserRelationService userRelationService = (com.melot.kkcore.relation.service.UserRelationService) MelotBeanFactory.getBean("kkUserRelationService");

        JsonArray jRoomList = new JsonArray();
        Set<Integer> resultIds = userRelationService.followedAndFans(userId);
        ...
        List<Integer> actorIds = new ArrayList<>(resultIds);
        List<ActorInfo> actorInfoList = actorService.getActorInfoBatchByIdList(actorIds);

        for (ActorInfo actorInfo : actorInfoList) {
            JsonObject roomJson = new JsonObject();
            com.melot.kkcore.actor.api.RoomInfo roomInfo = actorService.getRoomInfoByIdWithoutCache(actorInfo.getActorId());
            if (roomInfo.getLiveType() <= 0 || !roomInfo.getRoomSource().equals(roomSource)) {
                continue;
            }
            roomJson.addProperty("userId", actorInfo.getActorId());
            roomJson.addProperty("nickname", actorInfo.getNickName());
            roomJson.addProperty("gender", actorInfo.getGender());
            if (actorInfo.getPortrait() != null) {
                roomJson.addProperty("portrait_path_256", ConfigHelper.getHttpdir() + actorInfo.getPortrait() + "!256");
                roomJson.addProperty("portrait_path_128", ConfigHelper.getHttpdir() + actorInfo.getPortrait() + "!128");
            }
