...
import com.melot.kk.location.api.dto.LocationDTO;
...
public class LocationFunctions {
    ...
    /**
     * 	根据行政区划编码获取对应的gps经纬度坐标【51120140】
     */
    public JsonObject getLocation(JsonObject jsonObject, boolean checkTag, HttpServletRequest request) {
        ...
        try {
            ...
            LocationDTO locationDTO =  locationService.getLocationDTOByAreaCode(areaCode);
            if(locationDTO != null && StringUtils.isNotEmpty(locationDTO.getLat())
                    && StringUtils.isNotEmpty(locationDTO.getLng())){

                result.addProperty("lat", locationDTO.getLat());
                result.addProperty("lng", locationDTO.getLng());
            }
            result.addProperty("TagCode", TagCodeEnum.SUCCESS);
            return result;
        } catch (Exception e) {
