...
public class NewsV2Source {
	...
	private static final String HOT_VIDEO = "hot_video_cache";
	...
	private static final String VIDEO_NEWS = "VIDEO_NEWS";
	...
	/****************************   新版动态         ************************************/
	public static Set<String> getHotNews(int start, int offset, int type) {
		Jedis jedis = null;
		try {
			jedis = getInstance();
	...
	public static Set<Tuple> getVideoTopic(int start, int offset) {
		Jedis jedis = null;
		try {
			jedis = getInstance();
	...
	public static void delHot(int newsId) {
		...
		try {
			...
			jedis.zrem(HOT_VIDEO, String.valueOf(newsId));
		} catch (Exception e) {
			logger.error("NewsSource.delNews exception", e);
		} finally {
			if(jedis!=null) {
